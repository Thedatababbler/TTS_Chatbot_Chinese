...
from train_gpt import create_model
import torch.nn.functional as F
import copy
#from utils.utils import YParams
...
def prepare_generate_voice(args, flag):
...
def main(FLAG):
    ...
    while True:
        try:
            ...
            #print(new_text)
            arg2 = collections.namedtuple('args2', 
                                         ['yaml_conf', 
                                         'checkpoint',
                                         'label_dir',
                                         'output_dir',
                                         ...
                                         'alignment_dir',
                                         'target_acoustic_dir'],)
            args2 = arg2(yaml_conf = './hparams.yaml', 
                        checkpoint = './log_dir/gmm/Tacotron/Tacotron.ckpt-85000',
                        label_dir = 'test',
                        output_dir = './model_outputs/waifu',
                        text_input = ''.join(new_text),
                        hparams = None,
                        use_gl = True,
                        alignment_dir = None,
                        target_acoustic_dir = None,)
            #hparams = YParams(args2.yaml_conf)
            #os.makedirs(args2.output_dir, exist_ok=True)
            #synthesizer = Synthesizer(hparams, args2)
            if FLAG:
                syn = prepare_generate_voice(args2, FLAG)
                syn(new_text)
            else:
                syn(new_text)
            FLAG=False
            song = AudioSegment.from_wav(args2.output_dir + "/test.wav")
            play(song)
