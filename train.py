...
def train(hparams, output_dir, restore_path=""):
    ...
    restore_path_v1 = "log_dir/example/Tacotron"
    ...
    with tf.Session(config=config) as sess:
        try:
            ...
            if args.restore:
                try:
                    checkpoint_state = tf.train.get_checkpoint_state(restore_path_v1)
                    if (checkpoint_state and checkpoint_state.model_checkpoint_path):
                        global_saver.restore(sess, checkpoint_state.model_checkpoint_path)
