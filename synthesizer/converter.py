...
def get_pyin(text, tone=True):
    ...
    while i < len(text):
        ...
        if text[i] == '#':
            i += 1
            if i < len(text) and text[i] in ['1', '2', '3', '0']:
                res.append('#' + text[i])
                i += 1
    ...
    for j in range(len(seg_list)):
        ...
        for i in range(len(item)):
            if i < len(item) - 1: 
                ...
                new_item.append(item[i])
        seg_list[j] = ''.join(new_item)
        print(seg_list)
    text2 = '#2'.join(seg_list)
    return text2      
